...
OpenXrInterface::NamedPoseVelocity FilteredPosePublisher::filterJumps(const OpenXrInterface::NamedPoseVelocity &input)
{
    ...
    {
        yCWarningThrottle(OPENXRHEADSET, 5) << label() << "position had a jump. Last valid position: " << m_lastValidData.pose.position.transpose() << ", measured position: " << output.pose.position.transpose();
    ...
    {
        yCWarningThrottle(OPENXRHEADSET, 5) << label() << "rotation had a jump. Last valid rotation: " << m_lastValidData.pose.rotation.coeffs().transpose() << ", measured rotation: " << output.pose.rotation.coeffs().transpose();
